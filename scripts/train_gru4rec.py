"""
scripts/train_gru4rec.py — Train GRU4Rec from scratch on BGE-M3 content embeddings.

Architecture: GRU4RecModel
  - Two-layer GRU, hidden=256, input projected from BGE-M3 1024-dim.
  - No category stream, no auxiliary losses.
  - Scored by dot product of last hidden state with candidate projections.

Purpose: provides the recurrent baseline row in the benchmark comparison table.
  Completes the architectural progression:
    Content Baseline → GRU4Rec → SASRecF → DIF-SASRec

Output: data/gru4rec_pretrained.pt  (arch="gru4rec_v1")

Usage:
    python scripts/train_gru4rec.py               # 20 epochs
    python scripts/train_gru4rec.py --epochs 2    # quick smoke test
    python scripts/train_gru4rec.py --batch-size 1024  # if VRAM is tight
"""
import argparse
import json
import os
...
OUTPUT_PATH = os.path.join(DATA_DIR, "gru4rec_pretrained.pt")
CLEORA_PATH = os.path.join(DATA_DIR, "cleora_embeddings.npz")


def parse_args():
    p = argparse.ArgumentParser(
        description="Train GRU4Rec from scratch (no category stream)"
    )
    ...
    p.add_argument("--resume-from",  type=str, default=None,
                   help="Resume from a checkpoint (e.g. gru4rec_pretrained_epoch10.pt)")
    p.add_argument("--start-epoch",  type=int, default=1,
                   help="Epoch to start from when resuming (e.g. 11 after epoch-10 checkpoint)")
