...
\node[model]  (m_bge)    at (\pOneL, \rowModel) {BGE-M3\\(Semantic Encoder)};
...
\node[retrieval] (r_text)  at (\colSearchL, \rowPipe) {BGE-M3\\HNSW};
\node[retrieval] (r_kw)    at (\colSearchC, \rowPipe) {Tantivy\\BM25};
\node[retrieval] (r_vis)   at (\colSearchR, \rowPipe) {CLIP\\HNSW};
...
\node[task]   (p_seq)    at (\colRecR, \rowOut) {\textbf{DIF-SASRec}\\(Decoupled Attention)};
...
\caption{System architecture. \emph{Solid} arrows show the
  inference-time data flow. \emph{Dashed} arrows show feedback
  loops that update the user profile after each interaction.
  Phase~1 (\protect\colorbox{gray!4}{\strut gray zone}) learns
  multi-modal representations across textual (BGE-M3), visual
  (CLIP), and behavioral (Cleora) modalities. Phase~2 splits into
  two pipelines:
  \emph{Active Search} (\protect\colorbox{blue!4}{\strut blue zone})
  handles multi-modal queries via Adaptive Weighted RRF, while
  \emph{Recommendation} (\protect\colorbox{green!4}{\strut green zone})
  generates proactive suggestions through two independent paths:
  Pipeline~A (Cleora collaborative filtering $\to$ content veto)
  and Pipeline~B (semantic HNSW retrieval $\to$ content veto $\to$
  DIF-SASRec).}
\label{fig:arch}
\end{figure}
