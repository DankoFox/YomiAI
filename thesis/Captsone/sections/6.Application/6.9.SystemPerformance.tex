...
The streaming LLM assistant has the highest absolute latency (P50 3.2 seconds for a full 200-token response). Because it streams token-by-token, the user sees the first output token within approximately 200--400\,ms of the request, which keeps the interaction feel responsive.
...
The largest single VRAM consumer is Qwen2.5 at 1.8~GB, followed by the AgentPool at 1.18~GB.
...
Without the LLM assistant, VRAM usage drops to approximately 2.6~GB, which fits within a consumer GPU with 8~GB VRAM.
The FAISS flat index (3.2~GB RAM) is the largest RAM consumer; in a memory-constrained environment it can be replaced by the HNSW index at the cost of exact-reconstruction capability.
...
\begin{enumerate}
    ...
    \item ML model loading ($\approx$12\,s): BGE-M3 and the AgentPool (8 agents $\times$ DIF-SASRec) are loaded sequentially on the GPU.
