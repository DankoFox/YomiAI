...
A sampled negative protocol is adopted, following the conventions of SASRec~\cite{kang2018self} and BERT4Rec~\cite{sun2019bert4rec}.
...
\begin{itemize}
    \item HR@$k$ (Hit Rate): 1 if the ground-truth item appears in the top-$k$ ranked candidates, 0 otherwise. Averaged over all test users.
    \item NDCG@$k$ (Normalised Discounted Cumulative Gain): position-sensitive ranking quality, rewarding earlier hits more strongly.
    \item MRR@$k$ (Mean Reciprocal Rank): the reciprocal of the rank of the first relevant item, averaged across users.
\end{itemize}
...
\begin{enumerate}
    \item Random Baseline: uniform random ranking over the $N+1$ candidates. Provides a lower bound.
    \item Content Baseline: mean pooling of BGE-M3 embeddings over the user's training sequence, followed by cosine similarity ranking. Represents a simple semantic profile without sequential modelling.
    \item GRU-SeqDQN: a recurrent reinforcement-learning approach using a GRU-based policy network trained with Deep Q-Network objectives. Represents the prior recommendation module.
    \item DIF-SASRec (Pipeline B): the proposed Decoupled Information-Feature Self-Attentive Recommendation model with separate content and category attention streams.
    \item Pipeline A (Cleora + BGE-M3): behavioural co-purchase graph embeddings (Cleora) for candidate retrieval, re-ranked by BGE-M3 semantic similarity.
\end{enumerate}
...
Architecture     & Decoupled Information-Feature SASRec \\
...
Cleora index size & 375{,}439 items \\
