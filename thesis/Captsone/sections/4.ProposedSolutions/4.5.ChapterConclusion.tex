...
Section~\ref{sec:viz_improvements} replaced synchronous rendering with skeleton loading states, exposed the dual-pipeline recommendation architecture as two explicit sub-tabs (People Also Buy and You Might Like), and added an inline training visualisation showing the DIF-SASRec loss sparkline with pretrain-to-online phase annotation.
...
Section~\ref{sec:storage_improvements} introduced a dual FAISS index strategy (an HNSW index for approximate ANN search and a flat index for exact reconstruction), loaded via memory mapping to eliminate the 45-second full-load startup penalty.
