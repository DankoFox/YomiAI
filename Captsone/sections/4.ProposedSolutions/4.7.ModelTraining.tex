...
The key insight from Run~2 was that interaction density matters more than user count for this architecture.
...
\begin{enumerate}
    \item Per-epoch negative resampling.
    ...
    \item Batch size 2{,}048 with AMP.
    Automatic Mixed Precision (fp16 forward pass, fp32 gradient accumulation) halves activation memory, allowing batch size 2{,}048 and doubling GPU throughput compared to Run~2.
    AMP was present in Run~2 but a scheduler-ordering bug suppressed its benefit.

    \item AMP + LR scheduler ordering fix.
...
The final training losses across the three runs (3.35 for Run~1, 3.06 for Run~2, 5.34 for Run~3) cannot be compared directly.
