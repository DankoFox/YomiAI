...
\begin{itemize}
    \item \textbf{Dataset}: The Amazon Book Reviews dataset, comprising 3{,}080{,}829 catalogue items and 100{,}000 sampled evaluation users with held-out test interactions.
    \item \textbf{Recommendation}: Two independent recommendation pipelines: Pipeline~A using Cleora behavioural graph embeddings re-ranked by BGE-M3 profile similarity, and Pipeline~B using DIF-SASRec sequential modelling with per-user online fine-tuning.
...
\begin{itemize}
    \item \textbf{Large-scale online learning}: Per-user online fine-tuning of DIF-SASRec via sampled softmax gradient steps is implemented. The system does not address distributed policy optimisation across multiple concurrent users or multi-agent bandit settings.
    ...
    \item \textbf{Collaborative filtering beyond Cleora}: Traditional matrix factorization and neighborhood based collaborative filtering methods are not used; user behavior is represented only through the Cleora graph embedding.
