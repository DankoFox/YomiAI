...
The dominant failure mode is the Cleora catalogue boundary: 45\% of unserved users have a target item that lies outside the 375{,}280-item co-purchase graph, making it unreachable by Pipeline~A entirely.
...
\hline
\textbf{Quartile} & \textbf{Seq. length range} & \textbf{Users} & \textbf{Pipeline A} & \textbf{Pipeline B} & \textbf{$A \cup B$} \\
\hline
Q1 (sparse)   & $<$ 120       & 25{,}000 & 0.831 & 0.688 & 0.921 \\
Q2            & 120 -- 260    & 25{,}000 & 0.899 & 0.759 & 0.968 \\
Q3            & 261 -- 480    & 25{,}000 & 0.913 & 0.791 & 0.978 \\
Q4 (dense)    & $>$ 480       & 25{,}000 & 0.927 & 0.836 & 0.988 \\
\hline
\end{tabular}
\end{table}

Several patterns emerge.
Pipeline~A (Cleora + BGE-M3) maintains strong performance even for sparse users (HR@10 = 0.831 in Q1), because the Cleora co-purchase graph encodes community-level co-occurrence that does not depend on the individual user's sequence length.
Pipeline~B (DIF-SASRec) shows a steeper gradient across quartiles: HR@10 rises from 0.688 for sparse users to 0.836 for dense users, confirming that sequential modelling benefits more from longer, richer interaction histories.
The combined system narrows the performance gap across quartiles: the Q1–Q4 spread for $A \cup B$ is only 0.067 (0.921 to 0.988), compared to 0.148 for Pipeline~B alone.
This confirms that Pipeline~A acts as a reliable floor for sparse users while Pipeline~B provides additional signal for dense users — the complementarity is not merely additive but specifically compensatory.
