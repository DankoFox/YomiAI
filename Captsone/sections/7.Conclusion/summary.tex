...
The legacy GRU-SeqDQN recommendation module achieved HR@10 = 0.0823 — statistically indistinguishable from the random baseline of 0.1000 — due to the fixed-size GRU bottleneck and the intractable 3-million-item DQN action space.
...
The combined system achieves HR@10 = 0.9736 and NDCG@10 = 0.5571 on 100{,}000 held-out test users, a 124\% improvement in HR@10 over the Content Baseline (0.4346) and an 843\% improvement over GRU-SeqDQN (0.1031).
The result is stable across five random evaluation seeds (standard deviation $\sigma = 0.0024$).
Multilingual search performance reached Precision@10 = 0.5500 on long Vietnamese queries under BGE-M3, compared to 0.0500 under BLaIR — an 11$\times$ improvement.
End-to-end warm-cache search latency was reduced from 1{,}102\,ms to 59\,ms, an 18.6$\times$ improvement.

Chapter~6 presented the deployed book discovery application, demonstrating the end-to-end integration of all components: the login gate, the multimodal search interface (text, image, and fused), the dual-panel recommendation view with the DIF-SASRec input-sequence strip, the real-time profile panel with the Bellman-error loss sparkline, and the streaming Qwen2.5 book assistant.
...
Met &
Table~\ref{tab:main_results}: System ($A \cup B$) HR@10 = 0.9736, NDCG@10 = 0.5571 \\

Achieve substantial improvement over the prior GRU-SeqDQN baseline &
Met &
HR@10 increased from 0.1031 (GRU-SeqDQN) to 0.9736 (proposed), an 843\% improvement (Table~\ref{tab:main_results}) \\
...
Met &
Latency reduced from 1{,}102\,ms to 59\,ms (18.6$\times$ improvement, Table~\ref{tab:latency_stages}) \\

Demonstrate platform generality through an application case study &
Met &
Chapter~6 presents the deployed application integrating all platform modules \\
\hline
\end{tabular}
\end{table}

All five objectives are met.
The most significant result is the HR@10 improvement from 0.1031 to 0.9736, which closes the gap between the prior near-random recommender and a system capable of correctly identifying the relevant item in 97.4\% of test cases.
