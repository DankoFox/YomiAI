...
These methods were selected for their empirical performance, latency characteristics, and compatibility with the existing data platform.

Chapter~3 analysed the limitations of the prior system.
The legacy GRU-SeqDQN recommendation module produced near-random results at this catalogue scale, due to the fixed-size GRU bottleneck and the intractable 3-million-item DQN action space.
The BLaIR text encoder was trained on (user review, item description) pairs, making it suited to matching long descriptive text rather than the short conversational queries users actually type. Being monolingual English, it also returned near-zero precision on Vietnamese queries.
...
Chapter~6 presented the book discovery application, demonstrating the end-to-end integration of all components: the login gate, the multimodal search interface (text, image, and fused), the dual-panel recommendation view with the DIF-SASRec input-sequence strip, the real-time profile panel with the training loss sparkline, and the streaming Qwen2.5 book assistant.
...
Build and run an interactive application integrating all platform components &
Met &
Chapter~6 presents the book discovery application combining multimodal search, dual-pipeline recommendations, online DIF-SASRec fine-tuning, and the Qwen2.5 LLM assistant \\
\hline
\end{tabular}
\end{table}

All five objectives are met.
The clearest improvement is in recommendation quality: the prior GRU-SeqDQN approach produced near-random results, while both proposed pipelines achieve substantially higher retrieval accuracy on the same 100{,}000-user test set (Chapter~5, Table~\ref{tab:main_results}).
...
\begin{enumerate}
    ...
    This makes the per-card score badges on the You Might Like panel directly interpretable by users.

    \item \textbf{Dual-panel recommendation interface with per-pipeline attribution.}
    The frontend exposes two recommendation panels that directly mirror the two backend pipelines, with per-card layer tags (Cleora\,+\,BGE-M3, Cleora\,+\,CLIP, DIF-SASRec) and the DIF-SASRec input-sequence strip.
    ...
    \item \textbf{Training loss sparkline for training transparency.}
    ...
    Rather than passing raw Wikipedia or Google Books extracts to the Qwen2.5 assistant, the grounding pipeline uses the same BGE-M3 encoder used throughout the system to select the top-5 plot-relevant sentences from the retrieved text.
...
\begin{itemize}
    ...
    \item \textbf{Leveraging negative feedback for Cleora graph updates.}
    The current not-interested signal is stored as a per-user Redis blacklist and filters items from future recommendations, but is not yet propagated to the Cleora co-purchase graph.
    Incorporating not-interested events as negative edges in the hyperedge representation could improve candidate diversity by down-weighting co-purchase links that lead to items users have explicitly rejected.
