...
\hline
Data Ingestion    & Batch-only pipeline; no deduplication; no error recovery; shallow schema validation & Section~\ref{sec:ingestion_improvements} \\
\hline
Data Storage      & 612 ms P50 FAISS search latency; no programmatic indexes defined on MongoDB collections — full collection scans on all profile and interaction queries & Section~\ref{sec:storage_improvements} \\
\hline
...
\hline
Recommendation    & GRU-SeqDQN HR@10 = 0.0823 (near random); no content feature integration; no behavioural graph signal; large action-space convergence failure & Sections~\ref{sec:infra_improvements}--\ref{sec:storage_improvements} \\
\hline
...
The GRU-SeqDQN model's near-random performance (HR@10 = 0.0823) renders the recommendation feature effectively unusable.
