...
MongoDB stores per-user interaction logs in a schema-flexible document format, while Redis serves as an asynchronous interaction write-queue: user events are pushed via \texttt{RPUSH} and drained to MongoDB by a background worker via \texttt{BLPOP}, decoupling the high-frequency interaction write path from disk I/O.
Docker Compose orchestrates MongoDB and Redis as isolated services, ensuring reproducible deployment across environments.
