...
A user opens the application, enters their user ID on the login gate, and lands on the Active Search tab.
...
On response arrival, the placeholders are replaced by result cards showing the cover image, title, author, genre, and score badges: an RRF fusion badge on every card, plus a BGE-M3 badge for text-channel similarity and a CLIP badge for visual-channel similarity where applicable.
...
\caption{Active Search results (Mode 1). Each result card presents cover image, title, author, genre, score badges, and three interaction controls: \textit{Interested}, \textit{Ask AI} (streaming LLM popover), and \textit{Not Interested}. (a)~Text-only query shows RRF and BGE-M3 badges. (b)~Combined text and image query additionally shows a CLIP badge, demonstrating the multimodal fusion path.}
\label{fig:search_demo}
\end{figure}

Each result card has three interaction controls.
Interested logs a click event, updates the in-memory user profile, and triggers a DIF-SASRec gradient step.
Not Interested stores the item in a per-user blocked list and removes it from the recommendation panels immediately, without triggering a training step.
...
For image queries, the user clicks the camera icon button embedded in the search input to open an upload popover.
...
After a user accumulates at least five click interactions, the Recommendations tab (Mode 2) switches from cold-start to personalised mode.
A toast notification confirms the transition.
...
The input-sequence strip above both columns displays the user's most recent click interactions in chronological order, making the model's input visible.}
\label{fig:rec_demo}
\end{figure}

The input-sequence strip is displayed above both recommendation columns when personalised mode is active.
It shows the user's most recent click interactions, up to eight items in chronological order. This is the exact sequence DIF-SASRec reads when generating the next-item prediction, so the user can see a concrete reason why each item was recommended.

Recommendations refresh automatically every three new interactions.
...
Every click event triggers a sampled softmax gradient step on the user's personal DIF-SASRec checkpoint.
...
An information tooltip beside the chart title explains the expected scale difference: pretraining uses 512 hard negatives whereas online training samples from the full 3-million-item catalogue, which produces a structurally lower loss.
When a gradient step fires, a transient loss-decrease badge appears beside the current loss value for 2.5 seconds, confirming that the model has updated.
