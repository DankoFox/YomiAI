...
The user enters a user ID and submits; the frontend calls \texttt{POST /auth/check}. If the ID is found in MongoDB, the session begins immediately. If not, a confirmation prompt offers account creation via \texttt{POST /auth/create} or the option to re-enter a different ID.
...
Each book card has a Not Interested button.
Clicking it immediately removes the item from the current recommendation panels and sends a \texttt{POST /interact} request with \texttt{action = "not\_interested"}.
...
The frontend appends the returned loss value to the sparkline on response, giving sub-second visual confirmation that the model has updated.
...
To reduce hallucination and produce content-specific descriptions rather than generic summaries drawn from training knowledge, the assistant uses a two-stage grounding pipeline before LLM generation:

\begin{enumerate}
    \item Context retrieval: The system queries the Google Books API using \texttt{intitle:} and \texttt{inauthor:} operators for precision, falling back to a Wikipedia search with progressively broader queries (volume-specific $\to$ series-level $\to$ title-only) if Google Books returns no description.
    ...
    This step uses the same BGE-M3 encoder used throughout the system, and in practice suppresses author biography sentences and publisher marketing text in favour of plot-relevant content.
\end{enumerate}
...
It can be polled to check when the server has finished loading all components and is ready to accept requests.
