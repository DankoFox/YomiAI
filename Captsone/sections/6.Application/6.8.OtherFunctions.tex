...
When a user selects Add to Cart on any book card, the item is appended to the in-session cart array, the header count increments, and a success notification is shown.
...
A toggle switch in the header allows the frontend to operate in either Live mode (calls the FastAPI backend) or Mock mode (uses pre-defined in-memory data).
