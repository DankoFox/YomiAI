...
Table~\ref{tab:app_dataset_stats} summarises the key dataset statistics.
...
Text embedding index & BGE-M3 HNSW, 1{,}024-dim, 1.7M vectors (production) \\
Visual embedding index & CLIP HNSW, 512 dimensions \\
\hline
\end{tabular}
\end{table}

Each item record contains the item identifier (ASIN), title, author name, genre label, a short description, and a URL pointing to the cover image hosted on Amazon CDN.
Metadata is loaded at startup from the Parquet file into an in-memory pandas DataFrame indexed by ASIN, so individual item lookups for search result hydration are constant-time.
...
At runtime, additional interactions are written by authenticated users and stored in MongoDB; these drive both the per-user fine-tuning of DIF-SASRec checkpoints and the incremental Cleora co-purchase graph updates described in Section~\ref{sec:cleora_incremental}.
