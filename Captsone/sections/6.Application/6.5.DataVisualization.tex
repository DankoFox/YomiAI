...
On response arrival, each result is rendered as a card containing the cover image, title, author, genre tag(s), and relevance score badges (overall match, text similarity, and image similarity).
...
Each recommendation card carries a pipeline layer label, and items that are new since the previous refresh are highlighted for four seconds.
...
The \textbf{SASRec Loss} card renders an SVG sparkline of the full Bellman-error loss history as a filled area chart.
